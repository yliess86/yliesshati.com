...
export default function Skills ({ skills }: { skills: ISkill[] }) {
  const elements = skills.map((skill, index) =>
    <div key={index} className="flex flex-col md:grid md:grid-cols-2 gap-4">
      <Card className={`flex flex-col gap-2 ${index % 2 == 0 ? "md:order-first" : "md:order-last" }`}>
        <h3 className="font-bold text-xl">{skill.domain}</h3>
        <p>{skill.desc}</p>
      </Card>
      <div className={`flex flex-wrap justify-start content-start gap-4 ${index % 2 == 0 ? "md:order-last" : "md:order-first" }`}>
