...
export default function Awards ({ publications }: { publications: IPublication[] }) {
  const elements = publications.map((publication, index) =>
    <Card key={index} className="flex flex-col gap-2">
      <div className="overflow-hidden rounded-sm w-full h-96">
      ...
      <h4 className="flex items-center gap-2"><FaUserFriends className="inline mr-1" />{publication.authors.map((author, index) => <span key={index}>{author.name} {author.surname}</span>)}</h4>
